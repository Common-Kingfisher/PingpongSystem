...
另开一个终端启动独立的团体名单测试库（不会影响默认 `demo.db`）：
...
随后打开终端输出的 `/team-roster?tid=...` 地址。测试场预置 3 支队伍、9 名已分队队员和 2 名未分队队员，方便实际验证草稿保存、跨队调换、校验提示、确认冻结及撤销冻结；再次加 `--fresh` 可重置数据。
...
| 团体赛（TEAM）领域基础：队伍即 Entry、对抗与盘骨架、赛制规格（生产注册表为空） | ⚙️ 仅后端接口，无界面/编排；见 [团体赛领域基础与边界](docs/TEAM_DOMAIN.md) |
| 团体赛 Runtime：阵容绑定 → 一盘开始 → 录该盘大比分 → 团体总分累计 → 达到获胜盘数自动结束、未打的盘标记跳过 | ⚙️ 后端接口已可用（含权限字段），界面由前端工作线接入；见 [Team Runtime Contract](docs/TEAM_RUNTIME_CONTRACT.md) |
| 名单确认、分组抽签过场动画 | ✅ |
| Excel / CSV 预览、校验、确认导入 | ✅ |
| 三局两胜、11 分；大比分必填，可同时录入完整且一致的逐局小比分；小组赛仅在淘汰签表生成前可后补，之后按人工流程处理 | ✅ |
| 正常胜 2 / 正常负 1 / 弃权 0 | ✅ |
| 各小组独立设置出线人数 | ✅ |
| 晋级线极端同分时由主裁判人工裁定并留痕 | ✅ |
| 单淘汰、季军赛或并列季军 | ✅ |
...
| 正式/演示赛事隔离、正式赛事防误删 | ✅ |
| 比分请求防重复提交 | ✅ |
| 手机录分页 `/admin/t/:tid/score/:matchId`（大比分必填、逐局小比分可选、异常结果、防重复点击） | ⚙️ 交互层已可用；Auth 接线等 A 轨契约（见 [D 轨 Day 3](docs/WORKSTREAM_D.md#d-轨-day-3-实施结果)） |
| 双败 / 总决赛重置 | 不在本版范围 |
