...
export default function TeamRosterPage() {
  ...
  useEffect(() => {
    ...
    if (window.confirm('名单修改尚未保存，离开将放弃这些修改。是否继续？')) blocker.proceed()
    else blocker.reset()
  }, [blocker])
  useEffect(() => {
    if (!dialog) return
  ...
  return <div className="team-roster-page">
    <header className="roster-titlebar"><div><span className="eyebrow">TEAM ROSTER WORKBOOK</span><h1>{sheet.tournament.name} · 队伍与名单</h1></div><div className="roster-title-actions"><Link className="ribbon-btn" to={`/team-ties?tid=${tid}`}>团体对抗</Link><div className={`roster-lock ${locked ? 'locked' : ''}`}>{stageLocked ? '赛事已进入比赛阶段 · 名单锁定' : locked ? `已冻结${sheet.tournament.confirmed_at ? ` · ${sheet.tournament.confirmed_at}` : ''}` : dirty ? '有未保存更改' : '已保存'}</div></div></header>
