...
export default function TeamKnockoutPage() {
  ...
  const load = useCallback(async () => {
    ...
    try { const [nextTournament, nextKnockout] = await Promise.all([api.getTournament(tid), api.getTeamKnockout(tid)]); setTournament(nextTournament); setKnockout(nextKnockout) }
    catch (requestError) { setTournament(null); setKnockout(null); setError(messageOf(requestError)) }
  ...
  return <div className="page team-knockout-page">
    ...
    {!knockout.generated ? <section className="card team-ties-empty"><h2>尚未生成团体淘汰签</h2><p>请先完成全部小组的晋级确认。生成只创建双方已确定的首轮对抗，后续轮次待上游胜者产生后再由后端支持。</p><div className="button-row"><Link className="btn" to={`/team-qualification?tid=${tid}`}>前往晋级确认</Link><button className="btn primary" onClick={() => void generate()} disabled={generating}>{generating ? '正在生成…' : '生成首轮淘汰签'}</button></div></section> : <>
