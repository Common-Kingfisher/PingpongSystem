...
export default function TeamTiePage() {
  ...
  return <div className="page team-tie-page">
    ...
    <section className="team-tie-hero"><div><span className="eyebrow">TEAM TIE · {tie.stage}</span><h1>{tie.home_team.display_name} <i>VS</i> {tie.away_team.display_name}</h1><p>{tie.format.display_name ?? '赛制信息暂未登记'} · 盘次顺序由服务端返回</p></div><div className="team-total-score"><small>当前总比分</small><b>{tie.home_score} : {tie.away_score}</b><span>{tie.target_wins == null ? '目标胜场暂不可用' : `先达 ${tie.target_wins} 胜`}</span></div></section>
    ...
    <div className="button-row"><button className="btn" disabled={busy} onClick={() => void load()}>刷新</button><Link className="btn" to={`/players?tid=${tid}`}>返回赛事</Link></div>
