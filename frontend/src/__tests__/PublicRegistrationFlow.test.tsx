...
// 场景 8（管理端开关 ↔ Public 状态）已按 ownership 去重移除：
// 管理端「报名设置」由 master / C 轨提供（PR #61 的 /settings + TournamentSettingsPage，
// TEAM 不变量由 PR #63 收口），其行为与真实 API 调用分别由 C 轨的
// `__tests__/TournamentSettingsPage.test.tsx` 与 D 轨的
// `__tests__/AdminSettingsRouteAcceptance.test.tsx`（真实 App 路由级）覆盖。
// D 轨不再在 Public 测试里复刻管理端 UI 契约 —— 原实现依赖 D 版“开启报名”按钮，
// 已与 master 的「checkbox + 保存报名设置」契约漂移。
// Public 侧仍然只依赖唯一事实源 `TournamentOut.registration_enabled`（见场景 1 / 2 / 6 / 7）。
