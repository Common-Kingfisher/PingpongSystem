...
def _rubber_permissions(tie: dict, rubber: dict, has_playing: bool) -> dict:
    ...
    return {
        ...
        "can_start": tie_open and status == TeamRubberStatus.READY.value and not has_playing,
...
def _rubber_view(
    ...
    return {
        ...
        "permissions": _rubber_permissions(tie, rubber, has_playing),
        "lineup_options": {
...
def set_lineup(
    ...
    for entry in (entry_a, entry_b):
        if entry["status"] != ENTRY_STATUS_ACTIVE:
            raise TeamRuntimeError(f"队伍「{entry['display_name']}」已退出赛事，不能安排阵容", 409)
...
def start_rubber(
    ...
    repo.mark_team_rubber_playing(conn, rubber_id)
...
def record_rubber_score(
    ...
    if rubber["status"] != TeamRubberStatus.PLAYING.value:
        raise TeamRuntimeError(
            ...
            else ("本盘已经结束，本版不支持改分" if rubber["status"] == TeamRubberStatus.FINISHED.value else "本盘已跳过，不能录分"),
            409,
        )
    ...
    repo.mark_team_rubber_finished(conn, rubber_id, home_score, away_score, winner_entry_id)

    rubbers = repo.list_team_rubbers(conn, tie_id)
    if _playing_rubbers(rubbers):
        # 串行规则保证不会出现；真出现说明数据被绕过接口改写，拒绝继续而不是留脏状态。
        raise TeamRuntimeError("检测到同一对抗存在多盘同时进行，拒绝自动结算", 409)

    finished = [r for r in rubbers if r["status"] == TeamRubberStatus.FINISHED.value]
