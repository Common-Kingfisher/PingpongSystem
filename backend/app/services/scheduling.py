...
# 一次调度中最多回看"每组最近多少场已结束比赛"，用于判断哪些选手"刚打完"。
# A2 之后 matches 已有真实 finished_at，回看顺序按 finished_at 降序（缺失时按比赛 id 兜底）；
# 回看窗口大小按 球台数/小组数 估算一批可能同时结束的场次，上限 3。
# 该窗口只是软性调度启发式参数，不是"最短休息时间"规则（分钟级休息要求尚未冻结）。
