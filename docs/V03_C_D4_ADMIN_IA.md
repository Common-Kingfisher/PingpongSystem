...
- 小组出线人数在抽签页只读展示后端 `qualify_count`；真实逐组编辑入口位于“赛事设置 → 赛制与规则”。
...
- `PATCH /api/tournaments/{tid}/groups/{group_id}/qualification`
...
手工录入的运动员积分默认填写 `1000`；CSV 导入时积分可以留空，并由后端使用兼容默认值 `1000`。该默认值不代表运动员真实水平。
...
- 统一 Rules Settings API：小组数、默认晋级数、局制、每局分数、季军与名次规则目前只读；已生成小组的逐组晋级人数已有独立真实写入口。
